    ...
    def processing(self, filedir):
        if filedir.split('.')[-1] == 'txt':
            ...
            if self.rxncon_detected > 0:
                sys.exit(
                    'Found rxncon file in .csv Format. This is not supported, please export zu .xls or Quick Format in .txt')
    ...
    def check_directory_type(self):
        ...
        if self.rxncon_detected == 1:
            if self.sbtab_detected:
                raise RxnconParserError(
                    'Error, both SBtab and rxncon files detected in input directory! Please clean up the directory!')
            elif self.other_detected:
                raise RxnconParserError(
                    'Error, files of unknown format (neither SBtab nor rxncon) detected!')  # basti: fkt da doppelt spaeter
            else:
