...
class Commandline(object):
    ...
    def outputformat_formating(self, possibilities, default):
        self.outputformat = raw_input(
            'Please enter the output format. Possible are .{0} (default= .{1}):\n'.format(" & .".join(possibilities),
                                                                                          default))
...
class Parser(Commandline):
    ...
    def parse_SBtab2rxncon(self):
        ...
        for ob_ele in self.ob_list:
            if (ob_ele["type"] == 'ReactionList' and ob_ele['object'].table_name == 'Reaction definitions') or ob_ele[
                "type"] == 'rxnconReactionDefinition':
    ...
    def build_reaction_list(self, ob, indexes_dict, reaction_definition, format):
        ...
        for i, row in enumerate(ob['object'].getRows()):
            ...
            if format == 'standard':
                ...
                # new rxncon
                r_def = [reaction for reaction in reaction_definition if
                         row[indexes_dict['rea']].lower() == reaction['UID:Reaction'].lower()][0]
                ...
                reaction_list[-1]['ComponentA[DSR]'] = '{0}/{1}({2})'.format(row[indexes_dict['cad']],
                                                                             row[indexes_dict['cas']],
                                                                             row[indexes_dict['car']])
                reaction_list[-1]['ComponentB[DSR]'] = '{0}/{1}({2})'.format(row[indexes_dict['cbd']],
                                                                             row[indexes_dict['cbs']],
                                                                             row[indexes_dict['cbr']])
    ...
    def build_rxncon(self, ob_list, reaction_definition):
        ...
        for ob in ob_list:
            ...
            if ob['type'] == 'ReactionID' or ob['type'] == 'rxnconReactionList':
                indexes_dict = {
            ...
            if self.d.rxncon_sbtab_detected == 0:
                if ob['type'] == 'ReactionID':
                    # standard SBtab Format, the reaction_list here is called ReactionID, as Reaction List was allready
                    #  picked in SBtab
                    reaction_list = self.build_reaction_list(ob, indexes_dict, reaction_definition, "standard")
            else:
                # new rxncon format
                if ob['type'] == 'rxnconReactionList':
                    indexes_dict['uid'] = ob['object'].columns.index('!UID:Reaction')
                    indexes_dict['ss'] = ob['object'].columns.index('!SourceState')
                    indexes_dict['ps'] = ob['object'].columns.index('!ProductState')

                    # Save the data of these three columns to list of dictionaries
                    reaction_list = self.build_reaction_list(ob, indexes_dict, reaction_definition, "rxncon")
        ...
        return dict(reaction_list=reaction_list, contingency_list=contingency_list,
                    reaction_definition=reaction_definition)  # build rxncon dict
